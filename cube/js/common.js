...
<<<<<<< HEAD
	...
	createCube(wrapper ,countRows, countCols){ 	// Отрисовываем куб
		...
		this.wrapper.addEventListener('mouseleave',  this.mouseOutFromWrapper.bind(this));
	...
	mouseOutFromWrapper(){	//прячем кнопки
		let timer = setTimeout(()=>{
			this.hiddeBtn();
		},300);

		this.btnRemoveCols.addEventListener('mouseenter',() =>{
			clearTimeout(timer);
		});

		this.btnRemoveRows.addEventListener('mouseenter',() =>{
			clearTimeout(timer);
		});
	...
	createElement(name  , classes , innerHtml = null , parents){  //Функция для создания элемента
		...
		if(this.wrapBlock == this.wrapper){
			this.wrapBlock.appendChild(this.element);
		}
		else{
...
>>>>>>> 5494b42caf762e6fdc5039dc8f8f28fb18e9fb5d
